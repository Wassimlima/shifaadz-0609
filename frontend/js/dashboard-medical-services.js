/**
 * SHIFAA DZ — Medical Services Dashboard Controller
 * Professional Medical Services Panel
...
function updateDisplay() {
    if (!msUser) return;
    document.querySelectorAll('.ms-name-display, .user-info strong').forEach(el => el.textContent = msUser.name || 'مزود خدمات');
...
async function loadMSStats() {
    try {
        ...
        setEl('ms-welcome-meta', `${items.length} خدمة مفعلة حالياً`);
        setEl('stat-orders', '0');
...
async function loadServices() {
    ...
    try {
        ...
        if (!items.length) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--muted)">لا توجد خدمات مضافة. أضف خدمتك الأولى!</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--red)">فشل تحميل الخدمات</td></tr>';
    }
}
...
function openMsAdd() {
    _msEditId = null;
    document.getElementById('ms-inv-title').textContent = 'إضافة خدمة جديدة';
...
window.openMsEdit = function(service) {
    ...
    document.getElementById('ms-inv-title').textContent = 'تعديل الخدمة';
...
async function submitService(e) {
    ...
    try {
        ...
        showToast(_msEditId ? 'تم التحديث بنجاح' : 'تمت الإضافة بنجاح', 'success');
