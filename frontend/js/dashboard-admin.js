 ...
 * Version: 6.0 — Fully Functional Sections
 */
...
async function loadAdminStats() {
    try {
        ...
        const getV = (path, obj) => path.split('.').reduce((acc, part) => acc && acc[part], obj) || 0;

        setEl('stat-users', fmtNum(getV('users.total', d)));
        setEl('stat-pharmacies', fmtNum(getV('pharmacies.total', d)));
        setEl('stat-labs', fmtNum(getV('users.by_role.lab', d)));
        setEl('stat-medreps', fmtNum(getV('users.by_role.med_rep', d)));
        setEl('stat-medicines', fmtNum(getV('inventory.total', d)));
        setEl('stat-subscriptions', fmtNum(getV('pharmacies.verified', d)));
        setEl('stat-donations', fmtNum(getV('donations.available', d)));

        setEl('ov-users', fmtNum(getV('users.total', d)));
        setEl('ov-pharmacies', fmtNum(getV('pharmacies.total', d)));
        setEl('ov-labs', fmtNum(getV('users.by_role.lab', d)));
        setEl('ov-medreps', fmtNum(getV('users.by_role.med_rep', d)));
        setEl('ov-medservices', fmtNum(getV('users.by_role.medical_services', d)));
        setEl('ov-donations', fmtNum(getV('donations.available', d)));
...
async function loadAllLabs() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">جاري التحميل...</td></tr>';
    try {
        const res = await api.get('/labs/index.php', { limit: 100 }); // Assuming this exists or returns data
        ...
        tbody.innerHTML = list.map(l => `
            <tr>
                ...
                <td>${esc(l.wilaya)}</td>
                <td>${esc(l.phone)}</td>
                <td><span class="pro-badge green">مفتوح</span></td>
                <td><button class="pro-btn pro-btn-ghost pro-btn-sm">إدارة</button></td>
            </tr>
        `).join('');
    } catch { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">فشل التحميل</td></tr>'; }
...
async function loadAllDonations() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">جاري التحميل...</td></tr>';
    try {
        const res = await api.get('/donations/index.php', { limit: 100 });
        const list = res.donations || res.data || [];
        tbody.innerHTML = list.map(d => `
            <tr>
                <td><strong>${esc(d.item_name_ar || d.item_name)}</strong></td>
                <td>${esc(d.wilaya)}</td>
                <td>${statusBadge(d.is_available, 'متوفر', 'تم الاستلام', '#dbeafe', '#1d4ed8', '#f3f4f6', '#6b7280')}</td>
                <td style="font-size:.8rem">${formatDate(d.created_at)}</td>
                <td><button class="pro-btn pro-btn-ghost pro-btn-sm">عرض</button></td>
            </tr>
        `).join('');
    } catch { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">فشل التحميل</td></tr>'; }
}
...
async function loadAllMedReps() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">جاري التحميل...</td></tr>';
    try {
        const res = await api.get('/admin/users.php', { role: 'med_rep' }); // Reusing users API with filter
        const list = res.data || [];
...
function initUserManagement() {
    document.getElementById('user-search-btn')?.addEventListener('click', async () => {
        ...
        try {
            const res = await api.get('/admin/users.php', { q, role });
            const list = res.data || [];
            tbody.innerHTML = list.map(u => `
                <tr>
                    <td><strong>${esc(u.full_name)}</strong></td>
